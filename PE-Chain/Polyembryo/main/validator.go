...
func (bft *BFTProtocol) OnPrepareMsg(bftMsg types.BFTMessage) error {
	logBFT.Infof("[Node-%d-%d] (验证者) 阶段：%+v，OnPrepareMsg",
		bft.config.ShardId, bft.config.NodeId, bft.phase)
	if !bft.phase.IsNew(Phase{bftMsg.BlockNum, BFT_PREPARE}) { // 检查阶段 phase 相同就不用查看了
		return nil
	}
	...
	if bft.currentBlock == nil && bftMsg.BlockNum > 0 {
	...
	prepareVoteMsg := types.NewBFTMsg(types.MessageType_PREPAREVOTE, block, signedMsg.Serialize(), bitmap, bft.startphase.height)
	bft.mu.Lock()
	bft.phase.Switch(bftMsg.Block.Height, BFT_PREPARE)
	phase = bft.phase
	bft.mu.Unlock()
	go bft.TryGossip(phase, prepareVoteMsg, topics.ConsensusLeader)
	logBFT.Infof("[Node-%d-%d] (验证者) 准备阶段投票 block-%d-%d",
		bft.config.ShardId, bft.config.NodeId, bftMsg.Block.ShardId, bftMsg.BlockNum)
...
func (bft *BFTProtocol) OnPrecommitMsg(bftMsg types.BFTMessage) error {
	logBFT.Infof("[Node-%d-%d] (验证者) 阶段：%+v，OnPrecommitMsg",
		bft.config.ShardId, bft.config.NodeId, bft.phase)
	...
	signedMsg := bft.bls.Sign(msg)
	precommitVoteMsg := types.NewBFTMsg(types.MessageType_PRECOMMITVOTE, block, signedMsg.Serialize(), bitmap, bft.startphase.height)
	...
	logBFT.Infof("[Node-%d-%d] (验证者) 预提交阶段投票 block-%d-%d",
		bft.config.ShardId, bft.config.NodeId, bftMsg.Block.ShardId, bftMsg.BlockNum)
	return nil
}

func (bft *BFTProtocol) OnCommitMsg(bftMsg types.BFTMessage) error {
	logBFT.Infof("[Node-%d-%d] (验证者) 阶段：%+v, OnCommitMsg",
		bft.config.ShardId, bft.config.NodeId, bft.phase)
	...
	signedMsg := bft.bls.Sign(msg)
	commitVoteMsg := types.NewBFTMsg(types.MessageType_COMMITVOTE, block, signedMsg.Serialize(), bitmap, bft.startphase.height)
	...
	logBFT.Infof("[Node-%d-%d] (validator) 提交阶段投票 block-%d-%d",
		bft.config.ShardId, bft.config.NodeId, bftMsg.Block.ShardId, bftMsg.BlockNum)
