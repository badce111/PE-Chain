...
func PruneProofs(rootHash common.Hash, key []byte, proofDb ethdb.KeyValueReader, NodeProofDb ethdb.KeyValueWriter) { // 开始key为空 proofDb连续存各地址的proof NodeProofDb为空
	wantHash := rootHash
	if ok, _ := proofDb.Has(wantHash[:]); !ok {
		NodeProofDb.Put(binaryToCompact(key), wantHash[:]) // 递归终点 遇到不在存各地址proof的proofDb中的便加入NodeProofDb
		return
	}
	buf, _ := proofDb.Get(wantHash[:])
	tn, _ := decodeNode(wantHash[:], buf)
	switch n := tn.(type) {
	case *shortNode:
		nn := n.Val
		if hash, ok := nn.(hashNode); ok { // 如果shortNode存的是hashNode则是扩展节点就递归这个hashNode
			copy(wantHash[:], hash)
			PruneProofs(wantHash, append(key, n.Key...), proofDb, NodeProofDb)
		} else { // 不是hashNode则是叶子节点便加入NodeProofDb
			NodeProofDb.Put(binaryToCompact(key), buf)
		}

	case *fullNode: // 待改成循环
		ln := n.Children[0]
		if hash, ok := ln.(hashNode); ok {
			copy(wantHash[:], hash)
			PruneProofs(wantHash, append(key, 0), proofDb, NodeProofDb)
		} else {
			//lkey:=append(key, 0)
			//enc,_ := rlp.EncodeToBytes(ln)
			NodeProofDb.Put(binaryToCompact(key), buf)
		}
		rn := n.Children[1]
		if hash, ok := rn.(hashNode); ok {
			copy(wantHash[:], hash)
			PruneProofs(wantHash, append(key, 1), proofDb, NodeProofDb)
		} else {
			//rkey:=append(key, 1)
			//enc,_ := rlp.EncodeToBytes(rn)
			NodeProofDb.Put(binaryToCompact(key), buf)
		}
	case hashNode:
		NodeProofDb.Put(binaryToCompact(key), buf)
...
// todo(wx): go_proves
func (t *SecureTrie) Proves(keys [][]byte, fromLevel uint, proofDb ethdb.KeyValueWriter) error {
	return t.trie.Proves(keys, fromLevel, proofDb)
}

func (t *Trie) Proves(keys [][]byte, fromLevel uint, proofDb ethdb.KeyValueWriter) error {
	proof := memorydb.New()
	for _, key := range keys {
		t.Prove(key, fromLevel, proof)
	}
	var key []byte
	PruneProofs(t.Hash(), key, proof, proofDb)
	return nil
}
...
// 检查 merkle 证明。给定的证明必须包含具有给定根哈希的 trie 中键的值。
// 如果证明包含无效的 trie 节点或错误的值，VerifyFrove 将返回错误。
...
// todo(wx) go proves
func VerifyProofs(rootHash common.Hash, NodeproofDb ethdb.KeyValueStore) (*Trie, bool, error) {
	trie := new(Trie)
	it := NodeproofDb.NewIterator(nil, nil)
	for it.Next() {
		key, value := it.Key(), it.Value()
		if len(value) != 32 { // 凭长度感觉非hashNode
			node, err := decodeNode(nil, value)
			if err != nil { // decode出错 是hashNode
				node = hashNode(value)
				//panic("NodeproofDb error")
			}
			trie.TryUpdateNode(key, node)
		} else {
			//panic("NodeproofDb error")
			trie.TryUpdateNode(key, hashNode(value))
		}

	}
	if bytes.Equal(rootHash.Bytes(), trie.Hash().Bytes()) {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb ethdb.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
	...
	key, parent = keybytesToBinary(key), root
	for {
...
func hasRightElement(node node, key []byte) bool {
	pos, key := 0, keybytesToBinary(key)
	for node != nil {
		switch rn := node.(type) {
		case *fullNode:
			for i := key[pos] + 1; i < 2; i++ {
